...
You are tasked with conducting comprehensive READ-ONLY analysis of cloud deployments and infrastructure using cloud-specific CLI tools (az, aws, gcloud, porter, etc.).
...
1. Which cloud platform (Azure/AWS/GCP/Porter/other)
...
1. **Verify Cloud CLI Access:**
   - Check if the appropriate CLI is installed (az, aws, gcloud, porter)
...
6. **Generate Cloud Research Document:**
   ...
   ### Porter Configuration (if applicable)
   - Projects: [List]
   - Clusters: [List]
   - Applications: [List]
   - Services: [List]
...
7. **Save and Present Findings:**
   ...
   - Save to `thoughts/shared/cloud/CLNNN_platform_environment.md`
   - Create cost analysis in `thoughts/shared/cloud/costs/` if detailed
...
- **Use --output json** for structured data parsing where possible
...
### Porter (v2)
- Use `porter` CLI
- Verify context with `porter config`
- List projects: `porter projects list`
- List clusters: `porter clusters list`
- Inspect apps: `porter apps list` (or `porter app list` - verify with --help)
- Inspect services: `porter services list` (or `porter service list`)
- **Note**: Porter v2 uses a project/cluster context. Ensure correct context is set via `porter config set-project` or `porter config set-cluster` if navigating between them (only if needed for read access, otherwise list global).

## Error Handling:

- If CLI not authenticated: Guide user through login (e.g., `porter auth login`)
